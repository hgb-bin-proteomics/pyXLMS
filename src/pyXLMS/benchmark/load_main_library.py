...
def load_main_library(filename: str) -> dict[str, set[str]]:
    ...
    for _, row in df.iterrows():
        ...
        group_cell = row["group annotation in main library"]
...
def match_main_library(input_file: str, peptide_to_groups: dict[str, set[str]]) -> pd.DataFrame:
    ...
    sequences_df["Same Group (main library)"] = sequences_df.apply(
...
if __name__ == "__main__":
    ...
    peptide_groups = load_main_library(peptide_file)
    result = match_main_library(input_file, peptide_groups)

    result.to_csv("out_main_library.csv", sep="\t", index=False)
    print(result)

    counts = result["Same Group (main library)"].value_counts()
