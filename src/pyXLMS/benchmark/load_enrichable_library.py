...
def load_enrichable_library(filename: str) -> dict[str, set[str]]:
    ...
    for _, row in df.iterrows():
        ...
        group_cell = row["group annotation in enrichable library"]
...
def match_enrichable_library(input_file: str, peptide_to_groups: dict[str, set[str]]) -> pd.DataFrame:
    ...
    sequences_df["Same Group (enrichable library)"] = sequences_df.apply(
...
if __name__ == "__main__":
    ...
    peptide_groups = load_enrichable_library(peptide_file)
    result = match_enrichable_library(input_file, peptide_groups)

    result.to_csv("out_enrichable_library.csv", sep="\t", index=False)
    print(result)

    counts = result["Same Group (enrichable library)"].value_counts()
